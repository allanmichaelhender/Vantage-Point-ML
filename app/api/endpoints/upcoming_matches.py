...
async def run_heavy_sync():
    ...
    try:
        async with async_session() as session:
            # Get matches from Groq + ODDS api service
            ...
            result = await session.execute(
                select(PlayerState).where(PlayerState.player_id.in_(list(unique_ids)))
            )
            ...
            if db_matches:
                ...
                print(
                    f"✅ Sync completed successfully - {len(db_matches)} matches updated"
                )
            else:
                print("⚠️ Sync completed but no matches found")
...
async def get_live_dashboard(session: AsyncSession = Depends(get_db)):
    ...
    # Return matches and sync date (simplified - no status field)
    return {
...
# Manual sync endpoint triggered by button
@router.post("/sync/manual", dependencies=[Security(get_api_key)])
async def manual_sync():
    print("🚀 Manual sync triggered by user")
    try:
        await run_heavy_sync()
        return {"status": "success", "message": "Manual sync completed"}
    except Exception as e:
        return {"status": "error", "message": f"Manual sync failed: {str(e)}"}
