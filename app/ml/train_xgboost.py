...
def create_training_data(processor, NN_model, start_date="2015-01-01", end_date=None):
    ...
    cols_for_scaling = [c for c in combined_df.columns if c.startswith(('p1_', 'p2_')) and not c.endswith('_id') and 'vec' not in c]
    combined_df[cols_for_scaling] = processor.scaler.transform(combined_df[cols_for_scaling])
    ...
    with torch.no_grad():
        ...
        surf_embedding_map = {label: surf_weights[i].tolist() for i, label in enumerate(surf_labels)}
        ...
        surf_df = pd.DataFrame(surf_embedding_list, index=combined_df.index, columns=surf_cols)
...
def train_xgboost():
    ...
    NN_model.load_state_dict(torch.load('app/ml/models/tennis_encoder_initial.pt', weights_only=True))
    ...
    joblib.dump(XGBoost_model, 'app/ml/models/final_xgboost_model.pkl')
