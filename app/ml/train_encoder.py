...
def train_encoder():
    ...
    df = df[
        (df["tourney_date"] >= "2010-01-01") & (df["tourney_date"] < "2015-01-01")
    ].copy()
    ...
    train_db, validation_set_db = torch.utils.data.random_split(
        dataset, [train_size, validation_set_size]
    )
    ...
    num_players = len(
        joblib.load("app/ml/models/player_encoder.pkl").classes_
    )  # Uniqueness of players is preserved
    ...
    for epoch in range(10):
        ...
        for p1, p2, surf, stats, y in train_loader:  # Loading in a batch size of data
            ...
            loss = criterion(
                outputs, y
            )  # compares the model output to expected and calculates the loss over the batch
        ...
        print(
            f"Epoch {epoch + 1}/10 | Loss: {total_loss / len(train_loader):.4f} | Val Acc: {accuracy:.2%}"
        )  # Thats loss per bastch size here
