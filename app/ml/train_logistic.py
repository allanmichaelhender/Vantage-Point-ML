...
def train_logistic():
    ...
    # 3. Setup the Search
    # cv=5 means it splits your data 5 times to find the most stable winner
    ...
    print("\n--- LR Final Performance ---")
    print(f"✅ Accuracy: {accuracy:.2%}")
    print(classification_report(y_excluded, predictions))

    # 4. Save the Final Model
    # 🎯 Renamed to reflect the model type
    joblib.dump(lr_model, "app/ml/models/LogisticRegression.pkl")
    print("🏁 LR Model saved to app/ml/models/LogisticRegression.pkl")

    # 5. Get Feature Coefficients (Impact)
    # In LR, .coef_ shows the mathematical weight of each feature
    ...
    feature_importance_df = pd.DataFrame(
        {
            ...
            "importance": abs(
                importance
            ),  # Absolute value shows total impact on probability
        }
    ).sort_values(by="importance", ascending=False)

    print("\n--- Top 10 Most Impactful LR Features ---")
