...
const mergeMonthlyData = (
  ...
  return nn_hybrid.map((item, i) => ({
    ...
    // Neural Scout
    scout_profit: nn_scout[i]?.profit ?? 0,
    scout_roi: nn_scout[i]?.roi ?? 0,
    // Legacy (Logistic) 🎯 NEW
    legacy_profit: legacy[i]?.profit ?? 0,
...
export function MonthlyBreakdown({ data, globalModel }: Props) {
  ...
  const chartData =
    viewMode === "both"
      ? mergeMonthlyData(
          data.xgboost_nn.monthly_breakdown,
          data.xgboost.monthly_breakdown,
          data.nn.monthly_breakdown,
          data.logistic.monthly_breakdown, // 🎯 Add fourth arg
        )
      ...
      <div className="flex justify-between items-start mb-6">
        ...
        <div className="flex bg-slate-950/50 p-1 rounded-lg border border-slate-800 opacity-0 group-hover:opacity-100 transition-opacity">
          {["xgboost_nn", "xgboost", "nn", "logistic", "both"].map((mode) => {
            // 🧠 Map technical keys to clean Display Names
      ...
      <ResponsiveContainer width="100%" height="85%">
        <BarChart data={chartData} barGap={4}>
                ...
                return (
                  <div className="bg-slate-950 border border-slate-800 p-3 rounded-lg shadow-2xl space-y-3">
                    ...
                    {payload.map((entry: any) => {
                      ...
                      const roi = isNN
                        ? entry.payload.nn_roi
                        : entry.payload.base_roi || entry.payload.roi;
          ...
          {(viewMode === "xgboost_nn" || viewMode === "both") && (
            <Bar
              dataKey={viewMode === "both" ? "nn_profit" : "profit"}
              radius={[2, 2, 0, 0]}
            >
          ...
          {(viewMode === "xgboost" || viewMode === "both") && (
            <Bar
              dataKey={viewMode === "both" ? "base_profit" : "profit"}
              radius={[2, 2, 0, 0]}
            >
              {chartData.map((entry: any, index: number) => (
                <Cell
          ...
          {(viewMode === "nn" || viewMode === "both") && (
            <Bar
              dataKey={viewMode === "both" ? "scout_profit" : "profit"}
              radius={[2, 2, 0, 0]}
            >
              {chartData.map((entry: any, index: number) => (
                <Cell
          ...
          {(viewMode === "logistic" || viewMode === "both") && (
            <Bar
              dataKey={viewMode === "both" ? "legacy_profit" : "profit"}
              radius={[2, 2, 0, 0]}
            >
              {chartData.map((entry: any, index: number) => (
                <Cell
