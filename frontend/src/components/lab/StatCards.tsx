...
export function StatCards({ data, globalModel }: Props) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
      {items.map((item) => (
        <div key={item.label} className="bg-slate-900 border border-slate-800 p-6 rounded-xl shadow-sm transition-all duration-300">
          <div className="flex justify-between items-center">
            ...
            <span className="text-[8px] px-1.5 py-0.5 rounded bg-slate-800 text-slate-500 font-mono">
                {globalModel === 'xgboost_nn' ? 'NN' : 'BASE'}
            </span>
          </div>
          <p className={`text-2xl font-mono mt-2 ${item.color}`}>{item.value}</p>
        </div>
      ))}
    </div>
  );
