...
export default function LabPage() {
  ...
  if (error) return <div className="p-8 text-red-400 bg-slate-950 min-h-screen">Error: {error}</div>;
  if (!data) return null;
  ...
  return (
    <div className="min-h-screen bg-slate-950 p-8 text-slate-100 pb-20">
      <header className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
        <div>
          <h1 className="text-4xl font-black tracking-tight text-white">Model Lab</h1>
          <p className="text-slate-500 mt-1 font-medium">Quantitative Backtesting & Neural DNA Audit</p>
        </div>

        {/* 🎯 THE MASTER TOGGLE UI */}
        <div className="flex bg-slate-900 p-1 rounded-xl border border-slate-800 shadow-xl">
          {(['xgboost_nn', 'xgboost'] as const).map((m) => (
            <button
              key={m}
            ...
            >
              {m === 'xgboost_nn' ? 'Neural Hybrid' : 'Baseline Stats'}
            </button>
          ))}
        </div>
      </header>
